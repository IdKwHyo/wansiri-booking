// These checks run inside the same D1 batch transaction as the write.
// Named CHECK constraints abort the whole batch, including patient/audit inserts.
export function appointmentGuard(db:D1Database,eventId:string,a:{date:string;time:string;status:string;version?:number},id=''){
 ...
 return db.prepare(`WITH proposed(date,time,status,id,version) AS (VALUES(?,?,?,?,?))
 INSERT INTO mutation_guards(id,slot_ok,hours_ok)
 SELECT ?,CASE WHEN ${skip} THEN 1 ELSE
 ...
 CASE WHEN ${skip} THEN 1 ELSE p.time>=COALESCE((SELECT open FROM clinic_settings WHERE id=1),'10:00') AND p.time<COALESCE((SELECT close FROM clinic_settings WHERE id=1),'16:00') AND substr(p.time,4,2) IN ('00','30') END
...
export function settingsGuard(db:D1Database,eventId:string,s:{open:string;close:string;capacity:number;version:number},today:string){
...
export function clearGuard(db:D1Database,eventId:string){return db.prepare('DELETE FROM mutation_guards WHERE id=?').bind(eventId)}
